(ns teledyne-marine.main
  "Kotodama WASM entrypoint for the Teledyne_marine clean-room actor (L4) — Clojure port.
  ...
  facts (`teledyne_marine.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "teledyne_marine")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"teledyne_marine.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "teledyne_marine-compat" :tier tier :entities entities} 200])
...
(defn start! [] :teledyne_marine-compat/ready)
